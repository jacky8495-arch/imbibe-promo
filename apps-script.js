...
function getSheet() {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['LINE_UID', '顯示名稱', 'LIFF驗證時間', '驗證碼', '消費金額', '桌號', '人數', '核銷時間', '核銷狀態']);
...
function handleRedeem(e) {
  ...
  for (var i = data.length - 1; i >= 1; i--) {
    ...
    var rowRedeemed = data[i][8] || '';

    if (rowCode === code && rowDate === today && !rowRedeemed) {
      sheet.getRange(i + 1, 5).setValue(parseInt(amount));
      sheet.getRange(i + 1, 6).setValue(table);
      sheet.getRange(i + 1, 7).setValue(pax ? parseInt(pax) : '');
